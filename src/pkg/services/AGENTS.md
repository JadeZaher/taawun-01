...
- The cockpit's People view is derived from the same authorized membership store and exposes only active usernames, workspace roles, and join times. It never broadens workspace access or returns email/password fields.
- Workspace authorization reloads the actor and requires an active persisted account. This online check also invalidates signed Shura capabilities after an account is deleted, including for a workspace owner whose immutable audit records still name that user ID.
