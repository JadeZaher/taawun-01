...
|:---|:---|:---|:---|
| Vibecoding builder | Prompt/select, compose modules, theme, audit, preview, publish, and deploy | Central Datastar cockpit and APIs in progress | Complete authenticated happy path without code edits |
...
| Immutable artifacts | Versioned manifest, content-addressed assets, staging preview, tamper detection | Signed builder plus HTTP/MCP verification implemented | Integrated end-to-end lifecycle test |
...
| Taawun identity | Registration, login, workspace membership, secure sessions | Existing service under hardening | No default credentials; scoped access enforced |
...
| Shura governance | Architect/Maintainer/Viewer invitations, signed capabilities, revocation, audit trail | Signed capabilities, invitations, quorum, votes, decisions, JWKS, and audit implemented | Wire UI/card adapters and integrated authorization test |
...
| Relay infrastructure | Authenticated signaling, opaque encrypted payload transit, origin/rate/size controls | Hub and standalone binary implemented | Browser runtime integration and deployment documentation |
...
| Template Bazaar | Submit, stage, audit, approve, version, discover, install, and purchase | Track documents and financial scaffold only | Full sandbox lifecycle with authorization and anti-gharar preview |
| Donation and escrow | Sandbox intent, idempotency, approval, fail-closed settlement/refund, reconciliation | Durable vetted flows/provider boundary/audit implemented | UI/card adapter and integrated reconciliation test |
...
| Conductor | Observable build/audit/preview/publish stages | Legacy container-oriented track exists | Replace arbitrary container execution with artifact pipeline |
...
| Deployability | Central Railway service, persistent data, health/readiness, documented env | Canonical non-root/read-only Go 1.25 container implemented | Reproducible integrated build and verified Railway deployment |
