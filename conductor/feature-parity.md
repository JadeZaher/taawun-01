...
| Signed Starter Path | An empty-workspace organizer explicitly chooses a real template, customizes a component, verifies the exact signed preview and reaches a role-valid review handoff | Approved for implementation only: zero-history means a selected authorized workspace whose history loaded successfully with no rows; progress is derived ephemeral state, and invitation tokens remain session-only | Pass the promoted technical path without changing non-empty workspaces, then run the pending manual five-person comprehension study with real participants; the study does not block technical deployment |
